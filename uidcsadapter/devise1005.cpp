...
// 初始化静态成员变量
QMap<QPair<QString, QString>, QVariant>          DevIse1005::sm_mapDevNameInfo;         // 设备名和设备序列号映射

                                                                                        // 注册元类型
Q_DECLARE_METATYPE(tf::DeviceInfo)

DevIse1005::DevIse1005()
{
}
...
///
bool DevIse1005::RegisterDevice()
{
    // 构造查询条件
    ::tf::DeviceInfoQueryResp devResp;
    ::tf::DeviceInfoQueryCond devCond;
    devCond.__set_deviceType(tf::DeviceType::DEVICE_TYPE_ISE1005);
    ...
    {
        // 注册设备
        DYNAMIC_REGISTER_DEVICE(DevIse1005, QString::fromStdString(stuDevInfo.name), QString::fromStdString(stuDevInfo.groupName));
...
/// @li 4170/TangChuXian，2022年12月12日，新建函数
///
bool DevIse1005::OperateData(int iDataCode, int iDataOpCode, const QList<boost::any>& paramList, std::list<boost::any>& outBuffer)
{
    bool bRet = false;
    switch (iDataCode)
    {
...
/// @li 4170/TangChuXian，2022年12月12日，新建函数
///
bool DevIse1005::ExcuteCmd(int iCmdCode, const std::list<boost::any>& paramList, int& iRetCode)
{
    return true;
}
...
///
static boost::optional<ch::tf::QcApply> GetQcReason(ch::tf::QcType::type enQcType, const QString& strDevSn, int iModuleIdx, const tf::QcDoc& stuQcDoc)
{
    ::ch::tf::QcApplyQueryResp _return;
    ::ch::tf::QcApplyQueryCond qcapcd;
    qcapcd.__set_qcDocId(stuQcDoc.id);
    qcapcd.__set_deviceSN(strDevSn.toStdString());
    qcapcd.__set_assayCode(::ise::tf::g_ise_constants.ASSAY_CODE_ISE);
    qcapcd.__set_qcType(enQcType);
    qcapcd.__set_moduleIndex(iModuleIdx);
...
static boost::optional<ch::tf::QcApply> GetQcReason(const QC_APPLY_INFO& stuQcApplyItem)
{
    ...
    qcapcd.__set_moduleIndex(stuQcApplyItem.iModuleNo);
...
///
bool DevIse1005::OperateQcDocData(int iDataOpCode, const QList<boost::any>& paramList, std::list<boost::any>& outBuffer)
{
...
//         }
        ...
        for (auto& spQcDoc : allQcDocs)
        {
            ...
            // 如果不是ISE质控品，则跳过
            // （没有ISE项目）
            if (!IsQcDocMatchedRgnt(qcDoc, ::ise::tf::g_ise_constants.ASSAY_CODE_NA) &&
                !IsQcDocMatchedRgnt(qcDoc, ::ise::tf::g_ise_constants.ASSAY_CODE_K) &&
                !IsQcDocMatchedRgnt(qcDoc, ::ise::tf::g_ise_constants.ASSAY_CODE_CL))
            {
                continue;
            }
...
///
bool DevIse1005::OperateQcApplyData(int iDataOpCode, const QList<boost::any>& paramList, std::list<boost::any>& outBuffer)
{
    auto IsQcDocMatchedRgnt = [](const tf::QcDoc& stuQcDoc, int iAssayCode)
    {
        for (const auto& stuQcRgnt : stuQcDoc.compositions)
...
//         }
        ...
        // 缓存质控申请数据,索引为(项目通道号,(质控文档ID,模块索引))
        QMap<QPair<int, QPair<long long, int>>, QString> mapQcReason;
        QMap<QPair<int, QPair<long long, int>>, QPair<QString, QString>> mapQcApplyId;
        for (const auto& stuQcApp : _return.lstQcApplys)
        {
            ...
            auto itId = mapQcApplyId.find(qMakePair(stuQcApp.assayCode, qMakePair(stuQcApp.qcDocId, stuQcApp.moduleIndex)));
            if (itId == mapQcApplyId.end())
            {
                itId = mapQcApplyId.insert(qMakePair(stuQcApp.assayCode, qMakePair(stuQcApp.qcDocId, stuQcApp.moduleIndex)), QPair<QString, QString>());
            }

            // 如果是在用类型，直接插入质控原因
            if (stuQcApp.qcType == ch::tf::QcType::QC_TYPE_ONUSE)
            {
                mapQcReason.insert(qMakePair(stuQcApp.assayCode, qMakePair(stuQcApp.qcDocId, stuQcApp.moduleIndex)), ConvertTfEnumToQString(stuQcApp.qcReason));
            ...
            if (stuQcApp.qcType == ch::tf::QcType::QC_TYPE_DEFAULT)
            {
                ...
                if (mapQcReason.find(qMakePair(stuQcApp.assayCode, qMakePair(stuQcApp.qcDocId, stuQcApp.moduleIndex))) == mapQcReason.end())
                {
                    mapQcReason.insert(qMakePair(stuQcApp.assayCode, qMakePair(stuQcApp.qcDocId, stuQcApp.moduleIndex)), ConvertTfEnumToQString(tf::QcReason::QC_REASON_DEFAULT));
                }
                continue;
            }
        }
...
//         for (int i = 1; i <= stuDevInfo.moduleCount; i++)
//         {
//             // 如果模块号大于0且与i不匹配则跳过
//             if ((ModuleNo() > 0) && (ModuleNo() != i))
//             {
//                 continue;
//             }
// 
...
//                 }
// 
//                 // 质控品和试剂不匹配则跳过（没有ISE项目）
//                 if (!IsQcDocMatchedRgnt(stuQcDoc, ::ise::tf::g_ise_constants.ASSAY_CODE_NA) &&
//                     !IsQcDocMatchedRgnt(stuQcDoc, ::ise::tf::g_ise_constants.ASSAY_CODE_K) &&
//                     !IsQcDocMatchedRgnt(stuQcDoc, ::ise::tf::g_ise_constants.ASSAY_CODE_CL))
//                 {
//                     continue;
//                 }
// 
//                 // 获取项目信息
//                 std::shared_ptr<tf::GeneralAssayInfo> spAssayInfo = CommonInformationManager::GetInstance()->GetAssayInfo(::ise::tf::g_ise_constants.ASSAY_CODE_ISE);
//                 if (Q_NULLPTR == spAssayInfo)
//                 {
//                     continue;
//                 }
...
//         }
        ...
        // 组合质控申请信息
        //outBuffer = std::list<boost::any>(iOutBufferSize);
        QString strManApplyID("");
        QString strDefApplyID("");
        QString strQcReason("");
        QPair<int, QPair<long long, int>> pairAssayDoc;
        ...
        for (int i = 1; i <= stuDevInfo.moduleCount; i++)
        {
            // 如果模块号大于0且与i不匹配则跳过,0表示整机
            if ((ModuleNo() < 0) || 
				((ModuleNo() > 0) && (ModuleNo() != i)))
            {
                continue;
            }

            // 模块名相关信息赋值
            stuQcApplyInfo.iModuleNo = i;
            stuQcApplyInfo.strModuleName = (stuDevInfo.moduleCount > 1) ? (stuQcApplyInfo.strDevName + QString(QChar('A' + i - 1))) : stuQcApplyInfo.strDevName;
            stuQcApplyInfo.strID = stuQcApplyInfo.strDevName + "-" + QString::number(i);
            ...
            for (const auto& spQcDoc : allQcDocs)
            {
				...
				// 构造查询条件（项目通道号，（质控文档ID,模块号））
				pairAssayDoc.first = spAssayInfo->assayCode;
				pairAssayDoc.second.first = stuQcDoc.id;
				pairAssayDoc.second.second = stuQcApplyInfo.iModuleNo;
                ...
                stuQcApplyInfo.strID = stuQcApplyInfo.strID + "-" + stuQcApplyInfo.strQcDocID;
    ...
    if (iDataOpCode == int(DATA_OPERATE_TYPE_ADD))
    {
        ...
        // 获取质控ID
        long long lQcDocId = stuApplyInfo.strQcDocID.toLongLong();
        ...
        qcapcd.__set_moduleIndex(ModuleNo());
        qcapcd.__set_deviceSN(DeviceSn().toStdString());
        qcapcd.__set_assayCode(spAssayInfo->assayCode);
    ...
    if (iDataOpCode == int(DATA_OPERATE_TYPE_DELETE))
    {
        ...
        // 获取质控ID
        std::int64_t qcDocId = stuApplyInfo.strQcDocID.toLongLong();
...
/// @li 4170/TangChuXian，2022年12月19日，新建函数
///
bool DevIse1005::OperateDefaultQcData(int iDataOpCode, const QList<boost::any>& paramList, std::list<boost::any>& outBuffer)
{
    auto IsQcDocMatchedRgnt = [](const tf::QcDoc& stuQcDoc, int iAssayCode)
    {
        for (const auto& stuQcRgnt : stuQcDoc.compositions)
...
///
bool DevIse1005::OperateAssayNameListData(int iDataOpCode, const QList<boost::any>& paramList, std::list<boost::any>& outBuffer)
{
    // 清空输出缓冲区
    outBuffer.clear();
    ...
    if (iDataOpCode == int(DATA_OPERATE_TYPE_QUERY))
    {
        IseAssayIndexUniqueOrderCodeMaps mapAssayInfo;
        if (!CommonInformationManager::GetInstance()->GetIseAssayMaps((tf::DeviceType::type)DeviceType(), mapAssayInfo))
        {
            return false;
        }
        ...
        for (int i = 1; i <= stuDevInfo.moduleCount; i++)
        {
            ...
            {
                outBuffer.push_back(outData + "-" + DeviceName() + "-" + QString(QChar('A' + i - 1)));
            }
        }
        return true;
    }
...
///
bool DevIse1005::OperateQcLjAssayNameListData(int iDataOpCode, const QList<boost::any>& paramList, std::list<boost::any>& outBuffer)
{
    // 清空输出缓冲区
    outBuffer.clear();
...
/// @li 4170/TangChuXian，2023年8月28日，新建函数
///
bool DevIse1005::OperateQcTpAssayNameListData(int iDataOpCode, const QList<boost::any>& paramList, std::list<boost::any>& outBuffer)
{
    // 清空输出缓冲区
    outBuffer.clear();
...
///
bool DevIse1005::OperateQcConcInfoData(int iDataOpCode, const QList<boost::any>& paramList, std::list<boost::any>& outBuffer)
{
    // 获取质控品某个项目的靶值和标准差和CV
    ...
    if (iDataOpCode == int(DATA_OPERATE_TYPE_QUERY))
    {
        ...
        // 查询质控组合信息
        ::tf::QcDocQueryResp qcDocResp;
        if (!DcsControlProxy::GetInstance()->QueryQcDocByAssayCode(qcDocResp, spAssayInfo->assayCode) || qcDocResp.result != tf::ThriftResult::THRIFT_RESULT_SUCCESS)
        {
            return false;
        }
        ...
        for (const auto& stuDocInfo : qcDocResp.lstQcDocs)
        {
            ...
            qcCalcCond.__set_moduleIndex(ModuleNo());
            ...
            qcRltCond.__set_moduleIndex(ModuleNo());
...
/// @li 4170/TangChuXian，2022年12月28日，新建函数
///
bool DevIse1005::OperateQcRltInfoData(int iDataOpCode, const QList<boost::any>& paramList, std::list<boost::any>& outBuffer)
{
    // 清空输出缓冲区
    outBuffer.clear();
    ...
    if (iDataOpCode == int(DATA_OPERATE_TYPE_QUERY))
    {
        ...
        for (auto& stuData : qcRltResp.lstQcResults)
        {
			...
			stuQcRltInfo.strQcReason = ConvertTfEnumToQString(stuData.qcReason);
...
///
bool DevIse1005::OperateQcTargetValSDData(int iDataOpCode, const QList<boost::any>& paramList, std::list<boost::any>& outBuffer)
{
    // 清空输出缓冲区
    outBuffer.clear();
...
///
bool DevIse1005::OperateSingleQcReasonSolutionData(int iDataOpCode, const QList<boost::any>& paramList, std::list<boost::any>& outBuffer)
{
    // 清空输出缓冲区
    outBuffer.clear();
...
///
bool DevIse1005::OperateTwinQcReasonSolutionData(int iDataOpCode, const QList<boost::any>& paramList, std::list<boost::any>& outBuffer)
{
    // 清空输出缓冲区
    outBuffer.clear();
...
/// @li 4170/TangChuXian，2022年12月29日，新建函数
///
bool DevIse1005::OperateSingleQcCalcPointData(int iDataOpCode, const QList<boost::any>& paramList, std::list<boost::any>& outBuffer)
{
    // 清空输出缓冲区
    outBuffer.clear();
...
/// @li 4170/TangChuXian，2022年12月29日，新建函数
///
bool DevIse1005::OperateTwinQcCalcPointData(int iDataOpCode, const QList<boost::any>& paramList, std::list<boost::any>& outBuffer)
{
    // 清空输出缓冲区
    outBuffer.clear();
...
///
bool DevIse1005::OperateTwinQcRltInfoData(int iDataOpCode, const QList<boost::any>& paramList, std::list<boost::any>& outBuffer)
{
    // 清空输出缓冲区
    outBuffer.clear();
    ...
    if (iDataOpCode == int(DATA_OPERATE_TYPE_QUERY))
    {
        ...
        qryCond.__set_moduleIndex(ModuleNo());
        ...
        for (const auto& stuData : qryResp.lstQcResults)
        {
            ...
            // 查询X质控结果
            {
				...
				stuQcRltInfoX.strQcReason = ConvertTfEnumToQString(stuTfQcRltX.qcReason);
            ...
            // 查询Y质控结果
            {
				...
				stuQcRltInfoY.strQcReason = ConvertTfEnumToQString(stuTfQcRltY.qcReason);
...
///
int DevIse1005::ReagentSlotNumber()
{
    return 0;
}
