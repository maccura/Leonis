...
#include "ui_TrkIOMDebugWgt.h"
#include "shared/uicommon.h"
#include "src/common/Mlog/mlog.h"

TrkIOMDebugWgt::TrkIOMDebugWgt(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::TrkIOMDebugWgt)
    , m_isInit(false)
{
    ui->setupUi(this);
...
TrkIOMDebugWgt::~TrkIOMDebugWgt()
{
	ULOG(LOG_INFO, "%s", __FUNCTION__);
    delete ui;
}

void TrkIOMDebugWgt::showEvent(QShowEvent *event)
{
    // 基类处理
...
void TrkIOMDebugWgt::OnSelectListItem()
{
    // 记录之前选中的单元名称
    ...
    // 记录索引
    IOM::StackPageIdx enPageIdx = IOM::STACK_PAGE_IDX_STEP;
    QString pageName = ui->ModuleLstTbl->item(selIdx.row(), 0)->text();
    if(pageName == "电机")
    {
        enPageIdx = IOM::STACK_PAGE_IDX_STEP;
    }
    else if(pageName == "电磁铁")
    {
        enPageIdx = IOM::STACK_PAGE_IDX_ELECTROMAGNET;
    }
    else if(pageName == "扫码器")
    {
        enPageIdx = IOM::STACK_PAGE_IDX_SCANNER;
    }
    ui->stackedWidget_center->setCurrentIndex(enPageIdx);

}

void TrkIOMDebugWgt::InitUi()
{
    // 只有一列
...
void TrkIOMDebugWgt::InitData()
{
    ...
    ui->stackedWidget_center->insertWidget(IOM::STACK_PAGE_IDX_STEP, m_stepModuleWgt);
    ui->stackedWidget_center->insertWidget(IOM::STACK_PAGE_IDX_ELECTROMAGNET, m_magnetModuleWgt);
    ui->stackedWidget_center->insertWidget(IOM::STACK_PAGE_IDX_SCANNER, m_scanModuleWgt);
}

void TrkIOMDebugWgt::InitConnect()
{
    // 选中项改变
    QItemSelectionModel* pSelModel = ui->ModuleLstTbl->selectionModel();
    if (pSelModel != Q_NULLPTR)
    {
        connect(pSelModel, &QItemSelectionModel::selectionChanged, this, &TrkIOMDebugWgt::OnSelectListItem);
    }
    //注册消息

}

void TrkIOMDebugWgt::UpdateUi()
{

}

void TrkIOMDebugWgt::UpdateModuleList(ModuleInfo& info)
{
    //列表新建项
