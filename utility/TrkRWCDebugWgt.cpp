...
#include "ui_TrkRWCDebugWgt.h"
#include "shared/uicommon.h"
#include "src/common/Mlog/mlog.h"

TrkRWCDebugWgt::TrkRWCDebugWgt(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::TrkRWCDebugWgt)
{
    ui->setupUi(this);
...
TrkRWCDebugWgt::~TrkRWCDebugWgt()
{
	ULOG(LOG_INFO, "%s", __FUNCTION__);
    delete ui;
}

void TrkRWCDebugWgt::showEvent(QShowEvent *event)
{
    // 基类处理
...
void TrkRWCDebugWgt::OnSelectListItem()
{
    // 记录之前选中的单元名称
    ...
    // 记录索引
    RWC::StackPageIdx enPageIdx = RWC::STACK_PAGE_IDX_STEP;
    QString pageName = ui->ModuleLstTbl->item(selIdx.row(), 0)->text();
    if(pageName == "电机")
    {
        enPageIdx = RWC::STACK_PAGE_IDX_STEP;
    }
    else if(pageName == "电磁铁")
    {
        enPageIdx = RWC::STACK_PAGE_IDX_ELECTROMAGNET;
    }
	else if (pageName == "扫码器")
	{
		enPageIdx = RWC::STACK_PAGE_IDX_SCANNER;
	}
    ui->stackedWidget_center->setCurrentIndex(enPageIdx);

}

void TrkRWCDebugWgt::InitUi()
{
    // 只有一列
...
void TrkRWCDebugWgt::InitData()
{
    ...
    ui->stackedWidget_center->insertWidget(RWC::STACK_PAGE_IDX_STEP, m_stepModuleWgt);
    ui->stackedWidget_center->insertWidget(RWC::STACK_PAGE_IDX_ELECTROMAGNET, m_magnetModuleWgt);
    ui->stackedWidget_center->insertWidget(RWC::STACK_PAGE_IDX_SCANNER, m_scanModuleWgt);
}

void TrkRWCDebugWgt::InitConnect()
{
    // 选中项改变
    QItemSelectionModel* pSelModel = ui->ModuleLstTbl->selectionModel();
    if (pSelModel != Q_NULLPTR)
    {
        connect(pSelModel, &QItemSelectionModel::selectionChanged, this, &TrkRWCDebugWgt::OnSelectListItem);
    }
    //注册消息
}

void TrkRWCDebugWgt::UpdateUi()
{

}

void TrkRWCDebugWgt::UpdateModuleList(ModuleInfo& info)
{
    //列表新建项
