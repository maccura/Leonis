...
#include "ui_TrkRouterDebugWgt.h"
#include "shared/uicommon.h"
#include "src/common/Mlog/mlog.h"

TrkRouterDebugWgt::TrkRouterDebugWgt(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::TrkRouterDebugWgt)
{
    ui->setupUi(this);
...
TrkRouterDebugWgt::~TrkRouterDebugWgt()
{
	ULOG(LOG_INFO, "%s", __FUNCTION__);
    delete ui;
}

void TrkRouterDebugWgt::showEvent(QShowEvent *event)
{
    // 基类处理
...
void TrkRouterDebugWgt::OnSelectListItem()
{
    // 记录之前选中的单元名称
    ...
    // 记录索引
    Router::StackPageIdx enPageIdx = Router::STACK_PAGE_IDX_STEP;
    QString pageName = ui->ModuleLstTbl->item(selIdx.row(), 0)->text();
    if(pageName == "电机")
    {
        enPageIdx = Router::STACK_PAGE_IDX_STEP;
    }
    else if(pageName == "电磁铁")
    {
        enPageIdx = Router::STACK_PAGE_IDX_ELECTROMAGNET;
    }
    ui->stackedWidget_center->setCurrentIndex(enPageIdx);

}

void TrkRouterDebugWgt::InitUi()
{
    // 只有一列
...
void TrkRouterDebugWgt::InitData()
{
    m_stepModuleWgt = new TrkStepModuleWgt(this);
    m_magnetModuleWgt = new TrkMagnetModuleWgt(this);
    ui->stackedWidget_center->insertWidget(Router::STACK_PAGE_IDX_STEP, m_stepModuleWgt);
    ui->stackedWidget_center->insertWidget(Router::STACK_PAGE_IDX_ELECTROMAGNET, m_magnetModuleWgt);
}

void TrkRouterDebugWgt::InitConnect()
{
    // 选中项改变
    QItemSelectionModel* pSelModel = ui->ModuleLstTbl->selectionModel();
    if (pSelModel != Q_NULLPTR)
    {
        connect(pSelModel, &QItemSelectionModel::selectionChanged, this, &TrkRouterDebugWgt::OnSelectListItem);
    }
    //注册消息
}

void TrkRouterDebugWgt::UpdateUi()
{

}

void TrkRouterDebugWgt::UpdateModuleList(ModuleInfo& info)
{
    //列表新建项
